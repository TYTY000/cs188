...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state: Any):
        ...
        """
        "*** YOUR CODE HERE ***"
        return len(self.corners) == sum(state[1])
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if next_state in self.corners:
                    visited[self.corners.index(next_state)] = 1
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for corner in corners:
        manhattan = abs(x-corner[0]) + abs(y-corner[1])
        if manhattan > heur:
            heur = manhattan
    return heur
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    heur = 0
    for dot in dots:
        maze_dist = mazeDistance(position,dot,problem.startingGameState)
        if maze_dist > heur:
            heur = maze_dist
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        "*** YOUR CODE HERE ***"
        food_posi = self.food.asList()
        return state in food_posi
