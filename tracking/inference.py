...
def constructBayesNet(gameState: hunters.GameState):
    ...
    obs = []
    for x in range(X_RANGE+ Y_RANGE + MAX_NOISE -1):
        obs.append(x)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            cpt, joinFactor = joinFactorsByVariable(cpt, var)
            if len(joinFactor.unconditionedVariables()) > 1 :
                eliminateFactor = eliminate(joinFactor, var)
                cpt.append(eliminateFactor)
        
        return normalize(joinFactors(cpt))
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        "*** YOUR CODE HERE ***"
        vars = list(self.keys())
        prob = list(self.values())
        rand = random.choices(population = vars, weights = prob)
        return rand[0]
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        pacPosi = gameState.getPacmanPosition()
        jailPosi = self.getJailPosition()

        for posi in self.allPositions:
            self.beliefs[posi] *= self.getObservationProb(observation, pacPosi, posi, jailPosi)
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for posi in self.allPositions:
            transitionProb[posi] = self.getPositionDistribution(gameState, posi)

        for posi in self.allPositions:
            prob = 0
            for oldPosi in self.beliefs:
                prob += transitionProb[oldPosi][posi] * self.beliefs[oldPosi]
            newBelief[posi] = prob
        newBelief.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        particles = self.legalPositions * (self.numParticles // total)
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        pacPosi = gameState.getPacmanPosition()
        # ghostPosi = gameState.getGhostPosition()
        jailPosi = self.getJailPosition()
        belief = DiscreteDistribution()

        for particle in self.particles:
            prob = self.getObservationProb(observation, pacPosi, particle, jailPosi)
        ...
        if belief.total() == 0:
            ...
            for p in range(len(self.particles)):
                self.particles[p] = belief.sample()
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        for p in range(len(self.particles)):
            if particles[p] in belief:
                particles[p] = belief[particles[p]].sample()
            else:
                newPosDist = self.getPositionDistribution(gameState, particles[p])
                belief[particles[p]] = newPosDist
                particles[p] = newPosDist.sample()
        self.particles = particles
        "*** END YOUR CODE HERE ***"
