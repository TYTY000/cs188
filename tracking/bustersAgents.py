    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        ghostPosi = list(map(inference.DiscreteDistribution.argMax,livingGhostPositionDistributions))
        closestGhost = min(ghostPosi,key = lambda ghostP: self.distancer.getDistance(ghostP,pacmanPosition) )
        action = min(legal, key = lambda action: \
